...
const DEFAULT_DATA = {
  ...
  settings: {
    ...
    attendanceStarted: false, // Track if attendance has been started
    strictMode: false, // GPS location verification
    strictModeRadius: 500, // Meters
  },
...
export default {
  async fetch(request) {
    ...
    try {
      // GET /api/get-stats - Get attendance stats (only if attendance started)
      if (path === "/api/get-stats" && request.method === "GET") {
        const data = await getPastebinData()
        
        // Don't show stats if attendance hasn't started
      ...
      if (path === "/api/mark-attendance" && request.method === "POST") {
        ...
        // Check if already marked by this student today
        ...
        // Check if this device/IP tried to mark for different student today (FRAUD DETECTION)
        ...
        if (deviceAttendance.length > 0) {
          ...
          // If device is trying to mark for a different student
          if (existingStudent.studentId !== body.studentId) {
            ...
            // Second strike - BLACKLIST and void attendances
            if (fraudAttempts.length >= 1) {
              ...
              // Void both students' attendance for today
        ...
        // Mark attendance started when first student marks
        if (!data.settings.attendanceStarted) {
      ...
      // POST /api/log-fraud - Log fraud attempt
      if (path === "/api/log-fraud" && request.method === "POST") {
        const body = await request.json()
        const data = await getPastebinData()

        const attempt = {
          id: Math.random().toString(36).substring(2),
          attemptedStudent: body.attemptedStudent,
          actualStudentId: body.actualStudentId,
          ipInfo: body.ipInfo,
          deviceId: body.deviceId,
          timestamp: new Date().toISOString(),
        }
      ...
      // GET /api/admin/get-all-data - Get all data for admin
      ...
      if (path === "/api/admin/toggle-attendance" && request.method === "POST") {
        ...
        // If turning on attendance, mark it as started
        if (body.enabled) {
          data.settings.attendanceStarted = true
        }
      ...
      // POST /api/admin/bulk-add-students - Add multiple students
      ...
      if (path === "/api/admin/clear-attendance" && request.method === "POST") {
        ...
        data.attendance = []
        data.settings.attendanceStarted = false // Reset attendance started flag
        const success = await updatePastebin(data)
...
export default {
  async fetch(request) {
    ...
    try {
      // GET /api/get-stats - Get attendance stats
      if (path === "/api/get-stats" && request.method === "GET") {
        const data = await getPastebinData()
        const today = new Date().toISOString().split("T")[0]
        const todayAttendance = data.attendance.filter(a => a.date === today)
      ...
      if (path === "/api/mark-attendance" && request.method === "POST") {
        ...
        // Check if already marked (except protected students)
        if (!PROTECTED_STUDENTS.includes(body.studentId)) {
